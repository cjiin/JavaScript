...
// toSting() : 숫자형 데이터를 문자형 데이터로 변환해주는 함수 입니다. 
let x = 123;
x.toString();
console.log(x);
...
// undefined : 변수와 상수를 선언하면 생성한 저장공간에 데이터를 할당하는 것이 일반적인데 값이 할당이 되지 않으면 임시적으로 undefined 상태를 만듦.
let empty;
console.log(empty);

// null : null 값 하나만 있으며, 변수나 상수를 선언하고 의도적으로 선언한 공간을 비워둘 때 할당.
let empty2 = null;
console.log(empty2);